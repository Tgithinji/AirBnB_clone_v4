...
// Listen to changes on the span element next to the "Reviews" h2
const showReviewsButton = document.querySelector('#showReviews');
const reviewsList = document.querySelector('#reviewsList');

showReviewsButton.addEventListener('click', function () {
    if (showReviewsButton.textContent === 'show') {
        // Fetch and display reviews
        fetchReviews().then((reviews) => {
            displayReviews(reviews);
            showReviewsButton.textContent = 'hide';
        });
    } else {
        // Hide reviews
        reviewsList.innerHTML = '';
        showReviewsButton.textContent = 'show';
    }
});

// Function to fetch reviews (adjust the URL to your specific implementation)
...
// Function to display reviews in the reviewsList
